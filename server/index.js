...
function runAITurns(roomCode) {
  ...
  const G = room.G;
  const pi = G.cur;
  if(!G.players[pi].ai) { broadcast(roomCode); return; } // human turn — stop
  setTimeout(() => {
...
function handleAction(roomCode, pi, action) {
  ...
  if(type==='PLAY_HELD_RAT'){if(G.pile<=0)return;spend(cardId);zoneRat(G,target);if(target!==pi)G.touchedZones.push(target);log(room,`${pl.name} drops Held Rat into ${G.players[target].name}'s zone.`);broadcast(roomCode);}
