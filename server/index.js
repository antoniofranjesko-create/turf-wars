...
function broadcast(roomCode) {
  ...
  if(G && G.pendingReaction) {
    ...
    if(tgt && tgt.ai) {
      ...
      // AI block logic
      const blk = tgt.hand.some(c=>c.n==='Block') && Math.random()<(tgt.hp===1?0.9:0.6);
      if(blk) {
        tgt.hand.splice(tgt.hand.findIndex(c=>c.n==='Block'),1);
        G.discard.push({id:++_id2,n:'Block'});
...
function zoneRat(G, pi, room) {
  if(G.territorial && G.territorial.some(t=>t.owner===pi)) {
    log(room, `🐀 Rat blocked by Rat Territorial in ${G.players[pi].name}'s zone!`, 'sys');
    return;
  }
  if(G.pile > 0) G.pile--;
  addRat(G, pi, room);
...
function resolveWD(G, name, pi, room) {
  ...
  if(name === 'WD: Frenzy') { for(const p of alive) drawOne(G, p.i, room); }
    ...
    const pl = G.players[pi];
    if(pl.zone.length > 0) {
      pl.zone.splice(0,1); G.pile++;
      log(room, `🔍 WD: Health Inspection fires in ${pl.name}'s zone!`, 'dmg');
      damage(G, pi, 'WD: Health Inspection', room);
    } else {
...
function addRat(G, pi, room) {
  if(!G.players[pi].alive) return;
  G.players[pi].zone.push({ id: uid(), att: null, atts: [] });
...
function startTriggers(G, pi, room) {
  ...
  const preg = pl.zone.filter(r => r.atts && r.atts.some(a=>a.card.n==='Pregnant Rat'));
  for(const r of preg) { const pa=r.atts.find(a=>a.card.n==='Pregnant Rat'); r.atts=r.atts.filter(a=>a.card.n!=='Pregnant Rat'); if(pa&&pa.mode==='buff'&&pa.adoptTarget!=null){addRat(G,pa.adoptTarget,room);log(room,`Pregnant Rat births — newborn delivered to ${G.players[pa.adoptTarget].name}.`,'sys');}else{addRat(G,pi,room);} }
}
...
function resolveHI(G, atk, tgtI, ratZi, room) {
  ...
  for(let zi=T.zone.length-1; zi>=0; zi--) {
    ...
    if(!catBlocked) {
      ratToPile(G,tgtI,zi); hitsLanded++;
    }
  }
  if(hitsLanded>0) {
...
function runAITurn(G, pi, room) {
  ...
  for(let plays=0; plays<8 && !G.over; plays++) {
    ...
    // Rat Away
    if(has('Rat Away') && pl.zone.length && !G.ratAway?.includes(pi)) {
...
function handleAction(roomCode, pi, action) {
  ...
  if(type==='PLAY_PREGNANT_RAT'){const card=pl.hand.find(c=>c.id===cardId);if(!card)return;const tgt=G.players[target];if(!tgt||!tgt.zone.length)return;if(mode==='buff'){// buff: attach to own rat, newborn goes to adoptTarget's zone
    pl.hand.splice(pl.hand.indexOf(card),1);if(!pl.zone[ratZi||0].atts)pl.zone[ratZi||0].atts=[];pl.zone[ratZi||0].atts.push({card,placer:pi,type:'pregnant',mode:'buff',adoptTarget});log(room,`${pl.name} Pregnant Rat (buff) — newborn will go to ${G.players[adoptTarget].name}.`);broadcast(roomCode);}else{// nerf: attach to opponent's rat
    if(tgt.fortify)return;pl.hand.splice(pl.hand.indexOf(card),1);if(!tgt.zone[ratZi||0].atts)tgt.zone[ratZi||0].atts=[];tgt.zone[ratZi||0].atts.push({card,placer:pi,type:'pregnant',mode:'nerf'});G.touchedZones.push(target);log(room,`${pl.name} Pregnant Rat (nerf) on ${tgt.name}.`);broadcast(roomCode);}}
  ...
  if(type==='PLAY_FOOD_2') {
    ...
    const rat=pl.zone.splice(0,1)[0]; G.pile++;
  ...
  if(type==='PLAY_RAT_TERRITORIAL') {
    ...
    rat.atts.push({card:{id:uid(),n:'Rat Territorial'},placer:pi});
    G.territorial=(G.territorial||[]);
    G.territorial.push({owner:pi,ratId:rat.id});
    log(room,`${pl.name} plays Rat Territorial — no rats may enter their zone while this rat lives.`);
    broadcast(roomCode); return;
  }
  if(type==='PLAY_CAT') {
  ...
  if(type==='PLAY_RUSSIAN_ROULETTE') {
    ...
    if(boost && pl.zone.length>0) {
      pl.zone.splice(pl.zone.length-1,1); G.pile++;
