...
function zoneRat(G, pi, room) {
  // Check Rat Territorial — if owner has territorial rat, block entry
  if(G.territorial && G.territorial.some(t=>t.owner===pi)) {
    // rat blocked — return to pile (it was never taken out)
    log(room, `🐀 Rat blocked by Rat Territorial in ${G.players[pi].name}'s zone!`, 'sys');
    return; // pile not decremented since rat never left
  }
  if(G.pile > 0) G.pile--;
...
function resolveHI(G, atk, tgtI, ratZi, room) {
  ...
  // Cat protection: skip rats with Cat attachment
  const rat=T.zone[zi];
  if(rat && rat.atts && rat.atts.some(a=>a.card.n==='Cat')) {
    // consume the Cat
    const catIdx=rat.atts.findIndex(a=>a.card.n==='Cat');
    G.discard.push(rat.atts.splice(catIdx,1)[0].card);
    log(room,`😺 Cat protects ${T.name}'s rat from inspection!`,'sys');
    return;
  }
  ratToPile(G,tgtI,zi);
...
function handleAction(roomCode, pi, action) {
  ...
  if(type==='PLAY_HOT_RATATO') {
    ...
    const fixedDist = card && card.n==='Hot Ratato 2' ? 2 : card && card.n==='Hot Ratato 3' ? 3 : (dist||1);
    spend(cardId); if(boost && pl.zone.length>=2) spendFood();
    const cwIdx=getCWTarget(G,pi,fixedDist); if(cwIdx===-1) return;
  ...
  if(type==='PLAY_FOOD_2') {
    // spend 2 food, force all others to discard 1
    ...
    for(const o of alive) { if(o.hand.length){const ri=Math.floor(Math.random()*o.hand.length);G.discard.push(o.hand.splice(ri,1)[0]);} }
    log(room,`${pl.name} spends 2 Food — all others discard 1!`,'dmg');
  ...
  if(type==='PLAY_CAT') {
    ...
    if(!rat.atts) rat.atts=[];
    rat.atts.push({card,placer:pi});
    log(room,`😺 Cat attached to ${tgt.name}'s rat.`);
    broadcast(roomCode); return;
  }
