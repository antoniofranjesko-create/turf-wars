...
function handleAction(roomCode, pi, action) {
  ...
  if(type === 'END_TURN') {
    ...
    advanceTurn(G);
    if(!G.over) { startTriggers(G, G.cur, room); drawOne(G, G.cur, room); }
  ...
  if(type === 'PLAY_INHERITANCE') {
    spend(cardId); if(boost) spendFood();
    G.bounties = (G.bounties||[]);
    G.bounties.push({placer:pi, target, keep: boost?4:2});
    log(room, `${pl.name} puts a Inheritance on ${G.players[target].name}.`);
    broadcast(roomCode);
  }
...
io.on('connection', socket => {
  socket.on('create_room', ({ name, np }) => {
    ...
    console.log(`Room ${code} created by ${name}, waiting for ${npInt} players total`);
    // If 1P game (shouldn't happen but safety check)
    if(npInt === 1) {
      rooms[code].G = newGame(1, [name]);
      startTriggers(rooms[code].G, 0, rooms[code]);
  ...
  socket.on('join_room', ({ code, name }) => {
    ...
    // Rejoin by name if game already started
    if(room.G) {
